...
class Bert_CRF(BertPreTrainedModel):
    ...
    ```
    """
    def __init__(self, config, bio_num_labels=3, type_num_labels=3, output_attentions=False, keep_multihead_output=False):
        ...
        self.type_classifier = nn.Linear(config.hidden_size, self.type_num_labels)
        self.apply(self.init_bert_weights)

        tag_to_ix = {i:w for i,w in enumerate('BIO')}
        constraint_type = 'BIOE'
        #constrain = allowed_transitions(constraint_type=constraint_type, labels = tag_to_ix)
        constrain = [
                (0, 1),
                (1, 1), (1, 3),
                (2, 0), (2, 2), (2, 5),
        ...
        self.post = ConditionalRandomField(num_tags= len(constraint_type), constraints= constrain, include_start_end_transitions = True)

        self.criterion = nn.CrossEntropyLoss()

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, crf_mask=None, bio_labels=None, type_labels=None, head_mask=None, check=False):
        outputs = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, head_mask=head_mask)
        if self.output_attentions:
            all_attentions, sequence_output, _ = outputs
        else:
        ...
        # for training we do predict on each word
        # for testing we ensemble the last result
        type_logits = self.type_classifier(sequence_output)
        if(bio_labels is not None and type_labels is not None):
            masked = (type_labels>=0).byte().view(-1)
            index = torch.arange(0, masked.shape[0], dtype=torch.long).to(masked.device).masked_select(masked)

            tmp_type_logits = type_logits.view(-1, self.type_num_labels)[index]
            tmp_type_labels = type_labels.view(-1)[index]
            if(masked.sum().item()>0):
                type_loss = 3*self.criterion(tmp_type_logits, tmp_type_labels)
                expend_acc = (tmp_type_labels == tmp_type_logits.max(-1)[1]).cpu().float().mean().item()
            else:
                type_loss = 0
                expend_acc = 0
            ...
            if(check):
                ...
                return crf_loss+type_loss, {'expand_f1':bio_score, 'expand_acc':expend_acc}, {'bio':crf_pred, 'type':type_logits}
                #return type_loss, {'expand_f1':bio_score, 'expand_acc':expend_acc}, {'bio':crf_pred, 'type':type_logits}
                #return crf_loss, {'expand_f1':bio_score, 'expand_acc':expend_acc}, {'bio':crf_pred, 'type':type_logits}
        ...
        elif(bio_labels is None and type_labels is None):
            crf_pred = crf_result
            return {'bio':crf_pred, 'type':type_logits}
        else:
            raise ValueError('both should have value')
